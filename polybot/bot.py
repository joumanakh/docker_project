...
class ObjectDetectionBot(Bot):
    def handle_message(self, msg,images_bucket):
        ...
        if self.is_current_msg_photo(msg):
            ...
            # Define whimsical emojis for labels
            label_emoji = '\U0001f31f'

            label_counts = {}
            for label in labels:
                label_class = label['class']
                label_counts[label_class] = label_counts.get(label_class, 0) + 1

            # Format the message with emojis and counts
            message = "Detected objects:\n"
            for label_class, count in label_counts.items():
                message += f"{label_emoji} {label_class}: {count}\n"
            self.send_text(msg['chat']['id'], f'{message}')
            #*************************
